...
std::vector<std::vector<std::vector<float>>> load_volume_from_csv(const std::string& csv_file, int& rows, int& cols, int& depth) {
    ...
    for (int z = 0; z < depth; ++z) {
        for (int y = 0; y < rows; ++y) {
            if (!std::getline(file, line)) {
                std::cerr << "Error reading line for depth " << z << ", row " << y << std::endl;
                return {};
            }
            std::replace(line.begin(), line.end(), ',', ' ');
            std::istringstream row_stream(line);
            for (int x = 0; x < cols; ++x) {
                if (!(row_stream >> volume[z][y][x])) {
                    std::cerr << "Error parsing value at depth " << z << ", row " << y << ", col " << x << std::endl;
                    return {};
                }
            }
            // Check if there are extra values in the row
            if (row_stream >> line) {
                std::cerr << "Extra values found in depth " << z << ", row " << y << std::endl;
                return {};
            }
        }
    }
