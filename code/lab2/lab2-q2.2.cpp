             ...
             const real R, const int a)
{
    ...
    // Using the defined cache hash struct for both rows and columns
    std::unordered_map<int , double> r_cache;
    std::unordered_map<int , double> c_cache;
            ...
            for (int i = -a + 1; i <= a; ++i)
            {
                for (int j = -a + 1; j <= a; ++j)
                {
                    ...
                    if (m_in >= 0 && m_in < rows_in && n_in >= 0 && n_in < cols_in)
                    {
                        ...
                        // Check if the Lanczos value is in the row cache
                        auto it_r = r_cache.find(key_r);
                        // If the Lanczos value is not in the row cache
                        if (it_r == r_cache.end())
                        {
                            // Compute the Lanczos value and store it in the row cache
                            r_weight = lanczos(i, a);
                            r_cache[key_r] = r_weight;
                        }
                        else
                        {
                            // If the Lanczos value is in the row cache, set the weight to the value found in the cache
                            r_weight = it_r->second;
                        }

                        // Check if the Lanczos value is in the column cache
                        auto it_c = c_cache.find(key_c);
                        // If the Lanczos value is not in the column cache
                        if (it_c == c_cache.end())
                        {
                            // Compute the Lanczos value and store it in the column cache
                            c_weight = lanczos(j, a);
                            c_cache[key_c] = c_weight;
                        }
                        else
                        {
                            // If the Lanczos value is in the column cache, set the weight to the value found in the cache
                            c_weight = it_c->second;
                        }

                        weight = r_weight * c_weight;
