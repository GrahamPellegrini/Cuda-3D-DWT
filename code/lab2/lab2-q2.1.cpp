             ...
             const real R, const int a)
{
    ...
    // Defining both the pre defined Lanczos filter weights
    std::vector<double> r_weights(2 * a);
    std::vector<double> c_weights(2 * a );

    // Precomputing the Lanczos filter weights for the rows and columns
    for (int i = -a + 1; i <= a; ++i)
    {
        r_weights[i + a - 1] = lanczos(i, a);
        c_weights[i + a - 1] = lanczos(i, a);
    }
