...
void MonteCarloThread(const int N, int& local_count)
{
...
void MonteCarlo(const int N)
{
    ...
    // Create a vector to hold the count for each thread
    jbutil::vector<int, 128> counts(num_threads);
    // Note: Maximum number for the vectors is set to 128 to avoid memory allocation issues
    ...
    for (int i = 0; i < num_threads; ++i)
    {
        ...
        threads.push_back(std::thread(MonteCarloThread, samples_per_thread, std::ref(counts[i])));
    }
    ...
    int total_count = 0;
    // Loop over each local thread count and add to the total count
    for (int i = 0; i < num_threads; ++i)
    {
