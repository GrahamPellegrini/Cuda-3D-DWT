...
__global__ void row_kernel(float* data, float* temp, size_t filter_size, size_t depth_limit, size_t row_limit, size_t col_limit) {
    ...
    if (d < depth_limit && c < col_limit && i < row_limit / 2) {
        ...
        // Perform convolution using constant memory filters
        for (size_t j = 0; j < filter_size; ++j) {
            ...
            sum_low += lcf[j] * input_val;  // Use lcf for low-pass filter coefficients
            sum_high += hcf[j] * input_val; // Use hcf for high-pass filter coefficients
        }
