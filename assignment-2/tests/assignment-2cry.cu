...
void multi_level(float* d_volume, float* d_low_coeff, float* d_high_coeff, int levels, int& depth, int& rows, int& cols, int filter_size) {
    ...
    if (err != cudaSuccess) {
        std::cerr << "Failed to allocate GPU memory for final volume: " << cudaGetErrorString(err) << std::endl;
    }
    ...
    for (int i = 0; i < levels; i++) {
        ...
        err = cudaMalloc(&d_data2, depth * rows * cols * sizeof(float));
        if (err != cudaSuccess) {
        ...
        if (i < levels - 1) {
            // Allocate memory for the LLL subband
            float* d_subband = nullptr;
            int new_depth = depth / 2;
            int new_rows = rows / 2;
            int new_cols = cols / 2;

            cudaError_t err = cudaMalloc(&d_subband, new_depth * new_rows * new_cols * sizeof(float));
            if (err != cudaSuccess) {
                std::cerr << "Failed to allocate GPU memory for LLL subband: " << cudaGetErrorString(err) << std::endl;
            }
            dim3 gridDim((new_cols + blockDim.x - 1) / blockDim.x, (new_rows + blockDim.y - 1) / blockDim.y, (new_depth + blockDim.z - 1) / blockDim.z);
            // Extract the LLL subband
            extract_lll_subband<<<gridDim, blockDim>>>(d_data2, d_subband, depth, rows, cols);
            err = cudaDeviceSynchronize();
            if (err != cudaSuccess) {
                std::cerr << "Failed to synchronize after LLL subband extraction: " << cudaGetErrorString(err) << std::endl;
            }
            ...
            // Free previous volume memory
            cudaFree(d_data2);

            // Assign the LLL subband as the input for the next level
            d_volume = d_subband;
        } else {
            dim3 gridDim((init_cols + blockDim.x - 1) / blockDim.x, (init_rows + blockDim.y - 1) / blockDim.y, (init_depth + blockDim.z - 1) / blockDim.z);
            // Copy the transformed data back to the original volume
            copy_transformed_data<<<gridDim, blockDim>>>(d_data2, d_final, depth, rows, cols, init_depth, init_rows, init_cols);
            if (err != cudaSuccess) {
                std::cerr << "Failed to synchronize after copying transformed data: " << cudaGetErrorString(err) << std::endl;
            }

            // Free the temporary volume memory
            cudaFree(d_data2);
        }
