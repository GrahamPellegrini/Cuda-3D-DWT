...
void toGPU(std::vector<std::vector<std::vector<float>>> volume, size_t db_num, size_t depth, size_t rows, size_t cols, float*& d_low_coeff, float*& d_high_coeff, size_t&filter_size, float*& d_volume) {
