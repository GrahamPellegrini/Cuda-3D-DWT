...
__global__ void dwt_1d_kernel(
    ...
    if (idx < outer_size) {
        ...
        for (int i = 0; i < d_filter_length; ++i) {
            int global_idx = idx * stride_outer + i * stride_inner;
            shared_mem[threadIdx.x + i] = (global_idx < inner_size * stride_outer) 
                                          ? volume[global_idx] 
                                          : 0.0f; // Padding for boundary conditions
        }
    }
    __syncthreads();
    ...
    if (idx < outer_size) {
        ...
        float high_sum = 0.0f;
        for (int i = 0; i < d_filter_length; ++i) {
            low_sum += d_low_coeff[i] * shared_mem[threadIdx.x + i];
            high_sum += d_high_coeff[i] * shared_mem[threadIdx.x + i];
        }

        // Store the results in the output
        int out_idx = idx * stride_outer;
        volume[out_idx] = low_sum;
        volume[out_idx + stride_outer / 2] = high_sum;
    }
}
...
void dwt_3d(float* volume, int depth, int rows, int cols) {
    ...
    // Step 1: Row-wise DWT
    dim3 grid_dim_row((rows + block_dim.x - 1) / block_dim.x, depth); // Number of blocks
    size_t shared_mem_row = (block_dim.x + 16 - 1) * sizeof(float);
    dwt_1d_kernel<<<grid_dim_row, block_dim, shared_mem_row>>>(volume, cols, 1, rows, cols);
    cudaDeviceSynchronize();

    // Step 2: Column-wise DWT
    dim3 grid_dim_col((cols + block_dim.x - 1) / block_dim.x, depth); // Number of blocks
    size_t shared_mem_col = (block_dim.x + 16 - 1) * sizeof(float);
    dwt_1d_kernel<<<grid_dim_col, block_dim, shared_mem_col>>>(volume, 1, cols, cols, 1);
    cudaDeviceSynchronize();

    // Step 3: Depth-wise DWT
    dim3 grid_dim_depth((depth + block_dim.x - 1) / block_dim.x, rows); // Number of blocks
    size_t shared_mem_depth = (block_dim.x + 16 - 1) * sizeof(float);
    dwt_1d_kernel<<<grid_dim_depth, block_dim, shared_mem_depth>>>(volume, cols * rows, cols, depth, 1);
    cudaDeviceSynchronize();
