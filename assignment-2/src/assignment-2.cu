...
// Function to perform 1D DWT on a signal using CUDA with error checking
void dwt_1d_cuda(float* d_signal, int signal_size, const std::vector<float>& low_filter, const std::vector<float>& high_filter) {
    ...
    cudaCheckError(cudaMalloc(&d_low_filter, filter_length * sizeof(float)));
    cudaCheckError(cudaMalloc(&d_high_filter, filter_length * sizeof(float)));

    cudaCheckError(cudaMemcpy(d_low_filter, low_filter.data(), filter_length * sizeof(float), cudaMemcpyHostToDevice));
    cudaCheckError(cudaMemcpy(d_high_filter, high_filter.data(), filter_length * sizeof(float), cudaMemcpyHostToDevice));
    ...
    cudaCheckError(cudaFree(d_low_filter));
    cudaCheckError(cudaFree(d_high_filter));
}

// Function to perform 3D DWT on a 3D volume using CUDA with error checking
void dwt_3D_cuda(float* d_volume, int depth, int rows, int cols, int db_num) {
    ...
    for (int d = 0; d < depth; ++d) {
        ...
        for (int c = 0; c < cols; ++c) {
            ...
            float* d_column;
            cudaCheckError(cudaMalloc(&d_column, rows * sizeof(float)));
            cudaCheckError(cudaMemcpy(d_column, column.data(), rows * sizeof(float), cudaMemcpyHostToDevice));
            dwt_1d_cuda(d_column, rows, low_filter, high_filter);
            cudaCheckError(cudaMemcpy(column.data(), d_column, rows * sizeof(float), cudaMemcpyDeviceToHost));
            cudaCheckError(cudaFree(d_column));
            for (int r = 0; r < rows; ++r) {
    ...
    for (int r = 0; r < rows; ++r) {
        for (int c = 0; c < cols; ++c) {
            ...
            float* d_signal;
            cudaCheckError(cudaMalloc(&d_signal, depth * sizeof(float)));
            cudaCheckError(cudaMemcpy(d_signal, signal.data(), depth * sizeof(float), cudaMemcpyHostToDevice));
            dwt_1d_cuda(d_signal, depth, low_filter, high_filter);
            cudaCheckError(cudaMemcpy(signal.data(), d_signal, depth * sizeof(float), cudaMemcpyDeviceToHost));
            cudaCheckError(cudaFree(d_signal));
            for (int d = 0; d < depth; ++d) {
